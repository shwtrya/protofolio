  ...
  useGSAP(
    () => {
      ...
      mm.add('(max-width: 1023px)', () => {
        const tl = gsap.timeline();
        tl.from('.hero-socials', { y: -16, opacity: 0, duration: 0.7, ease: 'power2.out' })
          .from('.location', { y: 16, opacity: 0, duration: 0.7, ease: 'power2.out' }, '-=0.5')
          .from('.greetings', { y: 24, opacity: 0, duration: 0.7, ease: 'power2.out' }, '-=0.5')
          .from('.role', { y: 16, opacity: 0, duration: 0.7, ease: 'power2.out' }, '-=0.5')
          .from('.hero-resume', { y: 16, opacity: 0, duration: 0.7, ease: 'power2.out' }, '-=0.5')
          .from('.profile-card', { y: 30, opacity: 0, duration: 0.8, ease: 'power2.out' }, '-=0.5');
      });
    },
    { scope: heroRef }
  );
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-4xl">
        {/* Social Icons */}
        <div className="hero-socials mb-3 flex items-center gap-5 sm:mb-5 sm:gap-6 lg:mb-6">
          <a
            href={profile.github}
            target="_blank"
        ...
        {/* Location Tag */}
        <p className="location font-sans text-xs sm:text-sm uppercase tracking-[0.25em] text-[#111114]/60">
          Bogor · Indonesia
        </p>

        {/* Main Display Headline */}
        <h1 className="greetings mt-3 sm:mt-5 font-serif text-[clamp(2.75rem,8vw,8.5rem)] leading-[0.95] tracking-tight text-[#111114]">
        ...
        <div className="mt-2 sm:mt-3 inline-block">
          <div
            className="role block min-h-[1.75rem] sm:min-h-[2.5rem] lg:min-h-[3rem] text-lg sm:text-2xl lg:text-3xl xl:text-4xl font-light leading-tight tracking-[0.04em] text-[#111114]"
        ...
        {/* Action Button */}
        <div className="hero-resume mt-4 sm:mt-6 flex">
          <CvPreview
            className="inline-flex w-fit items-center rounded-full border border-[#111114]/15 bg-[#111114] px-5 py-2.5 sm:px-6 sm:py-3 font-mono text-[0.65rem] sm:text-xs font-semibold uppercase tracking-[0.18em] text-white transition-all duration-300 hover:bg-transparent hover:text-[#111114] cursor-pointer shadow-sm active:scale-95"
            label="Resume"
            showIcon={true}
          />
        </div>
      ...
      {/* RIGHT COLUMN: Profile Card (Responsive on mobile, 3D Tilt on desktop) */}
      <div className="profile-card relative z-10 flex w-full justify-center lg:justify-end [perspective:1200px]">
        <div
          ref={cardRef}
          ...
          style={{
            ...
            boxShadow: isHovered
              ...
              : '0 24px 60px rgba(17, 17, 20, 0.18)',
          }}
        >
          {/* Card background layers */}
          <div className="absolute inset-0 rounded-[1.75rem] sm:rounded-[2rem] bg-[linear-gradient(180deg,rgba(255,255,255,0.12),rgba(255,255,255,0)_34%),linear-gradient(180deg,#25252a_0%,#131418_48%,#050506_100%)]" />
          ...
          <svg
            ...
            fill="none"
            aria-hidden="true"
            className="size-8 absolute left-6 sm:left-8 top-20 sm:top-24 z-20 h-16 w-16 sm:h-20 sm:w-20 -rotate-12 text-white/25 pointer-events-none"
          >
            <path
          ...
          <svg
            ...
            fill="none"
            aria-hidden="true"
            className="size-8 absolute right-6 sm:right-8 top-14 sm:top-16 z-20 h-20 w-20 sm:h-24 sm:w-24 rotate-12 text-white/25 pointer-events-none"
          >
            <path
              d="M8.01 0.86L6.01 14.86L7.99 15.14L9.99 1.14L8.01 0.86Z"
              fill="currentColor"
          ...
          <img
            ...
            height={640}
            loading="eager"
            className="relative z-10 h-full w-full object-cover object-[50%_25%] rounded-[1.75rem] sm:rounded-[2rem] grayscale contrast-105 transition-all duration-700 hover:grayscale-0"
            style={{
              position: 'absolute',
              height: '100%',
              width: '100%',
          ...
          <div className="absolute inset-x-0 bottom-0 z-20 h-36 sm:h-40 rounded-b-[1.75rem] sm:rounded-b-[2rem] bg-gradient-to-t from-[#050506] via-[#050506]/80 to-transparent" />

          {/* Specular Glare */}
          {isHovered && (
            <div
              className="pointer-events-none absolute inset-0 z-20 rounded-[1.75rem] sm:rounded-[2rem] transition-opacity duration-300"
              style={{
      ...
      {/* Scroll Down Indicator matching iqmal.dev (hidden on small mobile screens to save space) */}
      <div className="hero-scroll-indicator absolute bottom-4 left-1/2 -translate-x-1/2 hidden sm:flex flex-col items-center gap-1.5 pointer-events-none opacity-45">
